...
from quizapp.forms import (StudentCreationForm,MyUserCreationForm,TeacherCreationForm,SubjectForm,StudentSubjectUpdateForm,TeacherSubjectUpdateForm,MyUserUpdateForm,
                           StudentUpdateForm,TeacherUpdateForm,QuizForm,AnswerForm)
    ...
    if request.method == 'POST' and user.is_teacher:
        ...
        if quizform.is_valid():
            ...
            if 'question_file' not in request.FILES and question_description is None:
                ...
                context = {'subject':subject,'subjects':subjects,'quizzes':quizzes,'quizform':quizform}
                return render(request,'quizapp/subject.html',context)            
            quiz = quizform.save(commit=False)
            quiz.subject = subject
            quiz.teacher = user.teacher
            quiz.save()
            return HttpResponseRedirect(reverse('subject', kwargs={'subject_slug' : subject_slug}))
...
def subject_people(request,subject_slug):
    ...
    context = {"teachers":teachers,"students":students,"subjects":subjects,"subject":subject}
...
def subject_grades(request,subject_slug):
    ...
    if user.is_student:
        subjects = user.student.subjects.all()
        answers = Answer.objects.filter(quiz__in = quizzes,student = user.student).order_by('-quiz__created_time')
        print(answers)
    if user.is_teacher:
        subjects = user.teacher.subjects.all()
    context = {'subjects':subjects,"subject":subject,'quizzes':quizzes,'answers':answers}
    # context = {}
...
def profile(request):
    ...
    context = {'user_form': user_form, 'profile_form': profile_form,'subjects':subjects,'profile':profile}
...
def quiz_detail(request,subject_slug,pk):
    ...
    context = {'subject':subject,'subjects':subjects,'quiz':quiz,'answer_form':answer_form,'answer':answer,'submitted':submitted,'submitted_answers':submitted_answers}
...
def mark_answer(request,pk):
    ...
    if user.is_student or (user.is_teacher and subject not in user.teacher.subjects.all()):
        return redirect('index')
